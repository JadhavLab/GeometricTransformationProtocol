function out = calclinfields_example(index, includeperiods, linpos, spikes, varargin)
...
%
%trajdata = FILTERCALCLINFIELDS(index, includeperiods, spikes, linpos, pos)
%trajdata = FILTERCALCLINFIELDS(index, includeperiods, spikes, linpos, pos, options)
%
...
%includeperiods - [start end] times for each include period
%
% options are
...
% modified by Wenbo Tang, Nov 5, 2020.
...
statevector(find(~isExcluded(statematrix.time, includeperiods))) = -1; % Based on exclude time, rather than inputing statevector as in calclinfields
