function [statematrix, segmenttable, trajwells, wellSegmentInfo, segmentInfo] = wb_linearizeposition_example(pos ,task, index, varargin)
...
branchpointdist = inf;
smoothwidth = 2;
do_dist = 1; % If set to 1, gets turnaround errors by distance travelled on arm
turndist = 0.75; % Fraction of arm length for getting turn-around error based on distance.
...
for option = 1:2:length(varargin)-1
    ...
    switch varargin{option}
        ...
        case 'headdir2' %there is already a variable in this function named headdir. Named headdir2 instead
            headdir2 = varargin{option+1};
        case 'minvelocity'
...
for i = 1:size(pos,1)
    ...
    if (pos(i,2) == 0)
        ...
        newpos(i, 2:3) = 0;
        warning(sprintf('zero position at index %d, day epoch %d %d', i, index(1), index(2)));
    else
        tmppos = [];
        ...
        if (lastvalid ~= -1)
            ...
            if (tmppos(lastid,3)- tmppos(newind,3)) < dismin
                newind = lastid;
            end
        else
...
% NOTE THAT TRAJMATRIX HAS THE RELEVANT INFO: Starttimes of traj, and Start-End Wells of Traj
try
    trajmatrix = gettraject(pos, welllocations,trajwells, lindist, wellSegmentInfo, welldist, mindiff, welldistthresh,branchpointdist,segmentInfo,do_dist,turndist); %,headdir2,minvelocity);
...
% Note: gettraj is internal function and is same as sj_getbehavestate but without velocity criterion.
% Default all includestates 1:6
[statematrix.traj, statematrix.lindist] = gettraj(statematrix, segmenttable, trajwells, minvelocity, headdir2);
...
segmentInfo.segmentTraj = segmentTrajectories; % Still oly 2 traj: 1 and 2
...
if do_dist==1
    ...
    if size(exittimes,1)>2
        ...
        for c=1:length(candidate)
            ...
            try
                currarmlth = 75; % plus maze, wells are 1, 2, 3, 4. hardcoded
            catch
                currarmlth = segmentInfo.segmentLength(candwell);
            end
...
%------------------------------------------------------------------
function [state, lindist] = gettraj(statematrix, segmenttable, trajwells, minvelocity, headdir2)
...
%Use linearVelocity to get forward and backward direction if only using
%one LED. Using segmentHeadDirections when you only have one LED will
%create an inaccurate linpos file.

if headdir2 == 1  %if head and motion direction should be in same direction
...
elseif headdir2 == 0 %if head and motion direction do not need to be in same direction
